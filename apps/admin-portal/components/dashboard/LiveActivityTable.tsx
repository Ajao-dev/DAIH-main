...
const activityRows: ActivityItem[] = [
  ...
  },
  {
    ...
    details: {
      ...
      notes: 'Microphone gain calibration and audio interface inspection needed.',
    },
  },
];
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-accent-soft">
                  <th className="py-2 font-label-md text-label-md text-outline font-medium">Time</th>
                  <th className="py-2 font-label-md text-label-md text-outline font-medium">Event</th>
                  <th className="py-2 font-label-md text-label-md text-outline font-medium">Entity</th>
                  <th className="py-2 font-label-md text-label-md text-outline font-medium text-right">Action</th>
                </tr>
              </thead>
              <tbody className="font-body-md text-body-md text-on-surface text-[14px]">
                {activityRows.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      <td className="py-2 text-on-surface-variant font-medium whitespace-nowrap">{row.time}</td>
                      <td className="py-2 whitespace-nowrap">
                        <span className="inline-flex items-center gap-1.5 font-medium">
                          <span className={`w-1.5 h-1.5 rounded-full ${row.dotColorClass}`} />
                          {row.event}
                        </span>
                      </td>
                      <td className="py-2 font-medium max-w-[150px] truncate">{row.entity}</td>
        ...
        >
          <div className="space-y-4 pt-2">
            <div className="flex items-center gap-2 p-3 bg-surface-container rounded-xl border border-accent-soft">
              <span className={`w-3 h-3 rounded-full ${selectedActivity.dotColorClass}`} />
              <div>
                <p className="text-sm font-bold text-on-surface">{selectedActivity.entity}</p>
                <p className="text-xs text-on-surface-variant">Logged {selectedActivity.time}</p>
              </div>
            </div>

            <div className="space-y-2 text-xs">
              <div>
                <span className="font-bold text-outline">Audit Reference: </span>
                <span className="font-mono text-primary font-semibold">{selectedActivity.details.reference}</span>
              </div>
              <div>
                <span className="font-bold text-outline">Device / Operator: </span>
                <span className="text-on-surface font-medium">{selectedActivity.details.operator}</span>
              </div>
