...
export const WalkInModal: React.FC<WalkInModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      toast.success(
        'Walk-in day pass issued successfully to ' + (formData.fullName || 'Guest') + '. Sequential Client ID generated.',
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Issue Walk-In Access Pass">
      <form onSubmit={handleSubmit} className="space-y-4 pt-2">
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-800 flex items-center gap-2">
          ...
          <span>This will create a temporary visitor badge and assign sequential Client ID for the day.</span>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label className="block text-xs font-bold text-on-surface mb-1">Access Package</label>
            <select
              value={formData.plan}
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-accent-soft">
          ...
          >
            ...
            <span>{isSubmitting ? 'Issuing Badge...' : 'Confirm & Issue Badge'}</span>
          </Button>
        </div>
      </form>
    </Modal>
