...
export const OccupancyOverview: React.FC<{ resources?: ResourceOccupancy[] }> = ({
  resources = defaultResources,
}) => {
  const totalSpots = resources.reduce((acc, curr) => acc + curr.totalCapacity, 0);
  ...
  return (
    <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            Real-time seat allotment across zones in Dare Adeboye Innovation Hub.
          </p>
        </div>

        <div className="flex items-center gap-3 bg-slate-50 px-3.5 py-1.5 rounded-xl border border-slate-200">
          <span className="text-xs font-semibold text-slate-600">Total Hub Occupancy:</span>
          <span className="text-sm font-extrabold text-[#23055c]">{overallPercentage}%</span>
          <span className="text-xs text-slate-400 font-mono">({totalOccupied}/{totalSpots})</span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        {resources.map((item) => {
          const percent = Math.round((item.occupied / item.totalCapacity) * 100);
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-3 text-xs">
                  <span className="font-semibold text-slate-700">
                    {item.occupied} / {item.totalCapacity} <span className="text-slate-400 font-normal">spots</span>
                  </span>
                  <span
                    className={cn(
