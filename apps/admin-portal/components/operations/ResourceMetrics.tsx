...
import { BarChart3, CalendarCheck, AlertTriangle, ArrowUpRight, ChevronRight, CheckCircle2 } from 'lucide-react';
...
export function ResourceMetrics({ resources, onOpenBlackouts }: ResourceMetricsProps) {
  const activeResources = resources.filter((r) => r.isActive);
  const totalCapacity = activeResources.reduce((acc, r) => acc + r.capacity, 0);

  // Find any active blackouts / maintenance
  const now = new Date();
  const maintenanceItems: { resource: FacilityResource; reason: string; endDate: string }[] = [];

  resources.forEach((r) => {
    (r.blackouts || []).forEach((b) => {
      if (b.isActive && new Date(b.startDate) <= now && new Date(b.endDate) >= now) {
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      ...
      <div className="bg-white border border-[#EBE7F5] rounded-xl p-6 shadow-xs flex flex-col hover:border-[#23055c]/20 transition-all">
        <div className="flex items-center gap-2 text-slate-500 mb-4">
          ...
          <h3 className="font-bold text-xs uppercase tracking-wider text-slate-600">Total Utilization</h3>
        </div>
        <div className="flex items-baseline gap-2 mb-2">
          <span className="text-4xl font-extrabold text-[#23055c] tracking-tight">{utilizationPercentage}%</span>
        ...
        <p className="text-xs text-slate-400 mt-auto">
          Across {activeResources.length} active resources ({totalCapacity} total capacity).
        </p>
      </div>
      ...
      <div className="bg-white border border-[#EBE7F5] rounded-xl p-6 shadow-xs flex flex-col hover:border-[#23055c]/20 transition-all">
        <div className="flex items-center gap-2 text-slate-500 mb-4">
          ...
          <h3 className="font-bold text-xs uppercase tracking-wider text-slate-600">Current Bookings</h3>
        </div>
        <div className="flex items-baseline gap-2 mb-2">
          <span className="text-4xl font-extrabold text-slate-900 tracking-tight">{activeBookingsCount}</span>
          <span className="text-sm font-semibold text-slate-400">/ {resources.length} resources</span>
        </div>
        <p className="text-xs text-slate-400 mt-auto">4 upcoming check-ins in the next hour.</p>
      </div>
        ...
        <div className="bg-[#ffdad6] border border-[#ba1a1a]/20 rounded-xl p-6 shadow-xs flex flex-col relative overflow-hidden">
          ...
          <div className="flex items-center justify-between mb-4 relative z-10">
            <div className="flex items-center gap-2 text-[#93000a]">
              ...
              <h3 className="font-bold text-xs uppercase tracking-wider">Maintenance Alerts</h3>
            </div>
        ...
        <div className="bg-emerald-50 border border-emerald-200/80 rounded-xl p-6 shadow-xs flex flex-col relative overflow-hidden">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2 text-emerald-900">
              ...
              <h3 className="font-bold text-xs uppercase tracking-wider">Facility Health</h3>
            </div>
