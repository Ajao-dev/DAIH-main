...
import { Edit2, Tag, CalendarOff, Power, Users, Wrench, Trash2, Clock } from 'lucide-react';
              ...
              return (
                <tr key={res.id} className="hover:bg-slate-50/80 transition-colors">
                  <td className="py-3.5 px-4 font-bold text-slate-900">
                    <div>{res.name}</div>
                    <div className="text-[10px] text-slate-400 font-mono font-normal">/{res.slug}</div>
                  </td>
                  ...
                  <td className="py-3.5 px-4 font-bold text-[#23055c]">
                    <span className="flex items-center gap-1">
                      <Users className="h-3.5 w-3.5 text-slate-400" /> {res.capacity} Max
                    </span>
                  </td>
                  <td className="py-3.5 px-4 text-slate-600 max-w-[160px] truncate">{res.location}</td>
                  <td className="py-3.5 px-4">
                    <div className="flex flex-wrap gap-1.5 max-w-xs">
                      {res.pricing && res.pricing.length > 0 ? (
                        res.pricing.map((p) => {
                          const currencySymbol = p.currency === 'NGN' ? '₦' : p.currency || '₦';
                          return (
                            <span
                              key={p.id}
                              className="px-2 py-0.5 rounded-full bg-blue-50 text-blue-700 text-[10px] font-semibold border border-blue-200/60"
                            >
                              {p.planName}: {currencySymbol}{Number(p.price).toLocaleString()}
                            </span>
                          );
                        })
                      ) : (
                    ...
                    <div className="flex items-center justify-end gap-1.5">
                      ...
                      <button
                        ...
                        className={`p-1.5 rounded-md transition-colors cursor-pointer ${
                          res.isActive ? 'text-slate-500 hover:bg-slate-100' : 'text-emerald-600 hover:bg-emerald-50'
                        }`}
                      >
