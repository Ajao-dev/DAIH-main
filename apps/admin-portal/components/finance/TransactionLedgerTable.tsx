...
import { Search, Filter, Eye, RotateCw, CheckCircle2, Clock, XCircle, ChevronLeft, ChevronRight, X, FileText, Check } from 'lucide-react';
...
export const TransactionLedgerTable: React.FC = () => {
  const [transactions, setTransactions] = useState<FinanceTransaction[]>(DEFAULT_TRANSACTIONS);
  ...
  const handleRetry = (tx: FinanceTransaction) => {
    toast.success(`Retrying Paystack charge for ${tx.reference}...`, { title: 'Payment Request Sent' });
  };
              ...
              <div className="absolute right-0 mt-2 w-44 bg-white border border-[#EBE7F5] rounded-xl shadow-lg py-1 z-50 animate-in fade-in zoom-in-95 duration-100">
                {['ALL', 'Paid', 'Pending', 'Failed'].map((status) => (
                  ...
                  >
                    ...
                    {statusFilter === status && <Check className="w-3.5 h-3.5 text-[#23055c]" />}
                  </button>
                ))}
              </div>
            )}
        ...
        <table className="w-full text-left border-collapse text-xs">
          <thead>
            <tr className="bg-[#F8F9FA] border-b border-[#EBE7F5]">
              <th className="py-3.5 px-6 font-bold text-slate-500 uppercase tracking-wider">Date</th>
              <th className="py-3.5 px-6 font-bold text-slate-500 uppercase tracking-wider">Member / Entity</th>
              <th className="py-3.5 px-6 font-bold text-slate-500 uppercase tracking-wider">Resource / Plan</th>
              <th className="py-3.5 px-6 font-bold text-slate-500 uppercase tracking-wider text-right">Amount</th>
              <th className="py-3.5 px-6 font-bold text-slate-500 uppercase tracking-wider text-center">Status</th>
              <th className="py-3.5 px-6 font-bold text-slate-500 uppercase tracking-wider text-right">Action</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[#EBE7F5]">
            {displayed.map((tx) => (
              <tr key={tx.id} className="hover:bg-slate-50/80 transition-colors group">
                <td className="py-4 px-6 text-slate-900 font-medium whitespace-nowrap">{tx.date}</td>
                <td className="py-4 px-6">
                  <div className="flex items-center gap-3">
                    <div className={`w-8 h-8 rounded-lg ${tx.avatarColorClass} flex items-center justify-center font-bold text-xs shrink-0 shadow-xs`}>
                      {tx.avatarLetter}
                    </div>
                    <div>
                      <p className="text-slate-900 font-bold">{tx.memberName}</p>
                      <p className="text-[11px] text-slate-400">{tx.memberEntity}</p>
                    </div>
                  </div>
                </td>
                <td className="py-4 px-6 text-slate-600 font-medium">{tx.resourcePlan}</td>
                <td className="py-4 px-6 text-slate-900 font-extrabold text-right">{tx.amount}</td>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-150">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-slate-200 relative">
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h4 className="font-bold text-slate-900 text-sm">Receipt & Invoice</h4>
                <p className="text-[11px] font-mono text-slate-400">{selectedTx.reference}</p>
              </div>
            </div>
            ...
            <div className="space-y-3 border-y border-slate-100 py-4 my-4 text-xs">
              <div className="flex justify-between">
                ...
                <span className="font-bold text-slate-900">{selectedTx.memberName} ({selectedTx.memberEntity})</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-medium">Workspace Resource:</span>
                <span className="font-semibold text-slate-800">{selectedTx.resourcePlan}</span>
              </div>
              <div className="flex justify-between">
            ...
            <div className="flex gap-2 justify-end">
              ...
              <button
                onClick={() => {
                  toast.success(`Printing official receipt for ${selectedTx.reference}`, { title: 'Receipt Downloaded' });
                  setSelectedTx(null);
                }}
