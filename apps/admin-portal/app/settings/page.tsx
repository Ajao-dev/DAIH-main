    ...
    <div className="space-y-8 max-w-5xl pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Configure operational policies, booking expiration windows, payment credentials, and system notifications.
          </p>
        </div>
      ...
      {isSaved && (
        <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-900 flex items-center gap-2.5 shadow-xs">
          ...
          <span className="text-xs font-bold">Settings saved successfully! Configuration changes are live.</span>
        </div>
      )}
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="bg-white rounded-2xl p-6 sm:p-8 border border-[#EBE7F5] shadow-xs space-y-5">
          <div className="flex items-center gap-2.5 border-b border-slate-100 pb-3">
            ...
            <h2 className="font-bold text-sm text-slate-900">Hub Identity & Contact Info</h2>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Input
              ...
              onChange={(e) => setSettings({ ...settings, hubEmail: e.target.value })}
            />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input
              ...
              onChange={(e) => setSettings({ ...settings, hubPhone: e.target.value })}
            />
            <Input
        ...
        <div className="bg-white rounded-2xl p-6 sm:p-8 border border-[#EBE7F5] shadow-xs space-y-5">
          <div className="flex items-center gap-2.5 border-b border-slate-100 pb-3">
            ...
            <h2 className="font-bold text-sm text-slate-900">Operational Hours & Booking Windows</h2>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <Input
              ...
              onChange={(e) => setSettings({ ...settings, weekendOpeningHour: e.target.value })}
            />
            <Input
              label="Weekend Closing"
              type="time"
              value={settings.weekendClosingHour}
              onChange={(e) => setSettings({ ...settings, weekendClosingHour: e.target.value })}
            />
          </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => setSettings({ ...settings, holdExpirationMinutes: parseInt(e.target.value) || 10 })}
                ...
                />
                <span className="text-xs text-slate-500 font-medium">minutes (releases unpaid desk locks)</span>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => setSettings({ ...settings, checkInGracePeriodMinutes: parseInt(e.target.value) || 15 })}
                  className="w-24 h-11 px-3.5 text-xs rounded-xl border border-slate-200 bg-white text-slate-800 focus:outline-none focus:ring-1 focus:ring-[#23055c] font-bold"
                />
                <span className="text-xs text-slate-500 font-medium">minutes (before automated hold release)</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl p-6 sm:p-8 border border-[#EBE7F5] shadow-xs space-y-5">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            <div className="flex items-center gap-2.5">
              ...
              <h2 className="font-bold text-sm text-slate-900">Paystack Gateway Configuration</h2>
            </div>
          ...
          <div className="space-y-4">
              ...
              <div className="flex gap-4">
                <label className="flex items-center gap-2 text-xs font-semibold text-slate-700 cursor-pointer">
                  <input
                    ...
                    onChange={() => setSettings({ ...settings, paystackMode: 'sandbox' })}
                ...
                <label className="flex items-center gap-2 text-xs font-semibold text-slate-700 cursor-pointer">
                  <input
                    ...
                    onChange={() => setSettings({ ...settings, paystackMode: 'live' })}
                    className="text-[#23055c]"
                  />
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-100 text-xs text-slate-600 font-mono flex items-center justify-between">
              <div>
                <span className="text-slate-400 font-sans block text-[10px] uppercase font-bold">Webhook Handler URL:</span>
        ...
        <div className="bg-white rounded-2xl p-6 sm:p-8 border border-[#EBE7F5] shadow-xs space-y-5">
          <div className="flex items-center gap-2.5 border-b border-slate-100 pb-3">
            ...
            <h2 className="font-bold text-sm text-slate-900">Alerts & System Notifications</h2>
          </div>
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <div>
                <span className="text-xs font-bold text-slate-800 block">Automated Email Receipts</span>
                <span className="text-[11px] text-slate-500">Send instant PDF receipt to members on verified Paystack transactions.</span>
              </div>
            </label>
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <div>
                <span className="text-xs font-bold text-slate-800 block">Critical System & Low Capacity SMS Alerts</span>
                <span className="text-[11px] text-slate-500">Notify Operations Admin when desk pool occupancy crosses 90%.</span>
              </div>
            </label>
          </div>
        </div>
