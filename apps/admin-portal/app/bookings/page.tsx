...
import {
  Calendar, Clock, Search, Filter, Plus, Loader2, RefreshCw,
  CheckCircle2, XCircle, AlertTriangle, ShieldCheck, User,
  ChevronLeft, ChevronRight, Ban, Eye, FileText
} from 'lucide-react';
...
export default function AdminBookingsPage() {
  ...
  // Release Hold Modal
  const [holdToRelease, setHoldToRelease] = useState<BookingSummary | null>(null);
  ...
  const handleConfirmReleaseHold = async () => {
    ...
    try {
      await api.bookings.adminReleaseHold(holdToRelease.id, releaseReason.trim() || undefined);
      toast.success(
  ...
  const handleCreateOverride = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!overrideForm.overrideReason || overrideForm.overrideReason.trim().length < 5) {
      toast.error('A clear override reason (minimum 5 characters) is mandatory for audit compliance.', {
    ...
    try {
      const startIso = new Date(`${overrideForm.startDate}T${overrideForm.startTime}:00`).toISOString();
      const endIso = new Date(`${overrideForm.endDate}T${overrideForm.endTime}:00`).toISOString();
      ...
      await api.bookings.adminOverride({
        ...
        totalAmount: overrideForm.waiveFee ? 0 : Number(overrideForm.totalAmount),
      });

      toast.success('VIP / Manual Reservation created and logged to Audit Trail.', {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Booking Engine Operations</h1>
          <p className="text-xs text-slate-500 mt-1">
            Real-time reservations console, occupancy tracking, and VIP manual overrides.
          </p>
        </div>
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs flex flex-col md:flex-row gap-4 justify-between items-center">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          {['ALL', 'CONFIRMED', 'HELD', 'PENDING_PAYMENT', 'CHECKED_IN', 'COMPLETED', 'CANCELLED', 'EXPIRED'].map((st) => (
            <button
              key={st}
              onClick={() => {
                setStatusFilter(st);
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs overflow-hidden">
        {loading ? (
          <div className="py-24 text-center space-y-3">
            ...
            <p className="text-xs text-slate-500 font-semibold">Loading booking reservations...</p>
          </div>
        ) : bookings.length === 0 ? (
          <div className="py-20 text-center space-y-3">
            <FileText className="h-8 w-8 text-slate-300 mx-auto" />
            <p className="text-sm font-bold text-slate-700">No bookings match your current filter</p>
            <p className="text-xs text-slate-400">Try changing status filter or clearing search.</p>
          </div>
        ) : (
                ...
                {bookings.map((b) => {
                  ...
                  return (
                    <tr key={b.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="py-3.5 px-4">
                        <div>{formatDate(b.startTime)} &mdash; {formatDate(b.endTime)}</div>
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between text-xs text-slate-500">
          <div>
            ...
            <span className="font-bold text-slate-800">{totalCount}</span> reservations
          </div>
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-1 min-w-0 flex-1">
                <h3 className="text-base font-bold text-slate-900">Force Release Active Hold</h3>
                <p className="text-xs text-slate-500">
                  Are you sure you want to release the hold for reservation{' '}
                  <span className="font-mono font-bold text-[#23055c]">{holdToRelease.reference}</span>?
                </p>
              </div>
            </div>
            ...
            <div className="bg-slate-50 rounded-xl p-3.5 border border-slate-200/80 space-y-1.5 text-xs text-slate-600">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-slate-800">{holdToRelease.resourceName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Customer:</span>
                <span className="font-semibold text-slate-800">{holdToRelease.customerName}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-400">Time Window:</span>
                <span className="font-semibold text-slate-800">
                  {formatTime(holdToRelease.startTime)} - {formatTime(holdToRelease.endTime)}
                </span>
              </div>
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              >
                {releasingHold ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-6 space-y-5 shadow-2xl border border-slate-200 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-base font-bold text-slate-900">Admin Manual Override</h3>
                  <p className="text-[11px] text-slate-500">Force reserve workspace with mandatory audit trail log</p>
                </div>
              </div>
              <button
            ...
            <form onSubmit={handleCreateOverride} className="space-y-4 text-xs">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setOverrideForm({ ...overrideForm, resourceId: e.target.value })}
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-600 uppercase mb-1">
                  Customer Email (Optional &mdash; leave empty for Walk-In / VIP)
                </label>
                <input
                  type="email"
                  placeholder="customer@example.com"
                  value={overrideForm.customerEmail}
                  onChange={(e) => setOverrideForm({ ...overrideForm, customerEmail: e.target.value })}
              ...
              <div>
                <label className="block text-[11px] font-bold text-slate-600 uppercase mb-1">
                  Mandatory Audit Override Reason <span className="text-rose-500">*</span>
                </label>
                <textarea
                  rows={2}
                ...
                <span className="text-[10px] text-slate-400">
                  This note will be permanently stamped in the database AuditLog.
                </span>
              </div>
              ...
              <div className="flex items-center gap-2 pt-1">
                <input
                  ...
                  onChange={(e) => setOverrideForm({ ...overrideForm, waiveFee: e.target.checked })}
                  className="rounded border-slate-300 text-[#23055c] focus:ring-[#23055c]"
                />
                <label htmlFor="waiveFee" className="text-xs font-semibold text-slate-700 cursor-pointer">
              ...
              <div className="flex gap-3 pt-3 border-t border-slate-100">
                ...
                >
                  {submittingOverride ? <Loader2 className="h-4 w-4 animate-spin" /> : null}
