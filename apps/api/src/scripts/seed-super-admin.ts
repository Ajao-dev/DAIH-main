...
async function seedSuperAdmin() {
  ...
  const { email, password, firstName, lastName, phoneNumber } = config.superAdmin;
  ...
  if (existing) {
    ...
    console.log(`✅ Super Administrator account updated successfully: ${updated.email} (${updated.clientId})`);
  } else {
    ...
    console.log(`✅ Super Administrator account created successfully: ${created.email} (${created.clientId})`);
  }
}
