...
function createRedisClient(): Redis {
  const isPlaceholder = config.redisUrl.includes('********') || !config.redisUrl;
  ...
  if (isPlaceholder) {
    safeLogger.info('Redis: Disabled (placeholder credentials in REDIS_URL) - Using in-memory store.');
    const dummy = new Redis({
      lazyConnect: true,
      enableOfflineQueue: false,
    });
...
redis.on('ready', () => {
  isConnected = true;
  safeLogger.info('Redis: Connected successfully (Distributed caching & rate-limiting active)');
});
...
redis.on('close', () => {
  if (isConnected) {
    isConnected = false;
    safeLogger.warn('Redis: Connection closed - Falling back to in-memory store');
  }
});
...
redis.on('error', (err) => {
  ...
  if (now - lastErrorLog > 30000) {
    const safeErrorMsg = sanitizeMessage(err?.message || 'Connection unreachable');
    safeLogger.warn(`Redis: Connection notice (${safeErrorMsg}) - Falling back to in-memory store`);
    lastErrorLog = now;
  }
});