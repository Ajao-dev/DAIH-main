...
import { sanitizeMessage, sanitizeObject, safeLogger } from '../utils/sanitizer.js';

// 1. Initialize Datadog Tracer (must run before instrumented libraries)
if (process.env.DD_API_KEY || process.env.DD_TRACE_ENABLED === 'true' || config.env === 'production') {
  tracer.init({
