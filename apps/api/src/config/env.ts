...
export const config = {
  ...
  databaseUrl: process.env.DATABASE_URL || 'postgresql://postgres:postgres@localhost:5432/daih_db?schema=public',
  redisUrl: process.env.REDIS_URL || 'redis://localhost:6379',
  jwt: {
    secret: process.env.JWT_SECRET || 'dev-secret-key-12345678901234567890',
    refreshSecret: process.env.JWT_REFRESH_SECRET || 'dev-refresh-secret-12345678901234567890',
    expiresIn: process.env.JWT_EXPIRES_IN || '15m',
    refreshExpiresIn: process.env.JWT_REFRESH_EXPIRES_IN || '7d',
    refreshExpiresInDays: parseInt(process.env.JWT_REFRESH_DAYS || '7', 10),
    verificationExpiresInHours: parseInt(process.env.VERIFICATION_EXPIRES_HOURS || '24', 10),
    passwordResetExpiresInHours: parseInt(process.env.PASSWORD_RESET_EXPIRES_HOURS || '1', 10),
  },
  cookies: {
  ...
  email: {
    ...
    resendFromEmail: process.env.RESEND_FROM_EMAIL || 'DAIH Hub <noreply@daih.ng>',
    zeptomailApiKey: process.env.ZEPTOMAIL_API_KEY || '',
    zeptomailFromEmail: process.env.ZEPTOMAIL_FROM_EMAIL || 'DAIH Hub <noreply@daih.ng>',
    zeptomailApiUrl: process.env.ZEPTOMAIL_API_URL || 'https://api.zeptomail.com/v1.1/email',
  },
  rateLimit: {
    loginMax: parseInt(process.env.RATE_LIMIT_LOGIN_MAX || '5', 10),
    loginWindowMinutes: parseInt(process.env.RATE_LIMIT_LOGIN_WINDOW || '15', 10),
    registerMax: parseInt(process.env.RATE_LIMIT_REGISTER_MAX || '10', 10),
    registerWindowMinutes: parseInt(process.env.RATE_LIMIT_REGISTER_WINDOW || '60', 10),
    verifyResendMax: parseInt(process.env.RATE_LIMIT_VERIFY_MAX || '3', 10),
    verifyResendWindowMinutes: parseInt(process.env.RATE_LIMIT_VERIFY_WINDOW || '60', 10),
    passwordResetMax: parseInt(process.env.RATE_LIMIT_RESET_MAX || '3', 10),
    passwordResetWindowMinutes: parseInt(process.env.RATE_LIMIT_RESET_WINDOW || '60', 10),
  },
  observability: {
  ...
  qrSigningSecret: process.env.QR_SIGNING_SECRET || 'dev-qr-signing-key-1234567890',
  paystack: {
