...
export const PricingPlanIdParamSchema = z.object({
  planId: z.string().trim().min(1, 'Plan ID is required').transform(sanitizeString),
});

export const BlackoutIdParamSchema = z.object({
  blackoutId: z.string().trim().min(1, 'Blackout ID is required').transform(sanitizeString),
});
...
export const CreateResourceSchema = z.object({
  ...
  description: z.string().min(5, 'Description must be at least 5 characters').transform(sanitizeString),
...
export const CreatePricingPlanSchema = z.object({
  planName: z.string().min(2, 'Plan name is required').transform(sanitizeString),
...
export const CreateBlackoutSchema = z.object({
  ...
  reason: z.string().min(3, 'Reason must be at least 3 characters').transform(sanitizeString),
  isActive: z.boolean().optional().default(true),
});
