...
describe('Calendar Availability API', () => {
  ...
  afterAll(async () => {
    if (testResourceId) {
      ...
      await prisma.facilityResource.deleteMany({ where: { id: testResourceId } });
    }
  });

  it('should return sparse calendar availability map with default AVAILABLE status', async () => {
    const res = await request(app)
      .get(`/api/v1/bookings/calendar-availability?resourceId=${testResourceId}&month=2026-08`);
  ...
  it('should reject invalid month format', async () => {
    const res = await request(app)
      .get(`/api/v1/bookings/calendar-availability?resourceId=${testResourceId}&month=invalid-month`);
