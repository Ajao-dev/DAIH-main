...
debugRouter.use((_req, res, next) => {
  if (config.env === 'production' && process.env.ENABLE_PROD_DEBUG !== 'true') {
    res.status(404).json({ code: 'NOT_FOUND', message: 'Debug routes disabled in production' });
    return;
  }
  next();
});
...
debugRouter.get('/sentry-error', (_req, _res, next) => {
  try {
    throw new Error('DAIH Smoke Test Exception — Milestone 1.1 Sentry Verification');
  } catch (error) {
